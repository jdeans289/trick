...
class SimTestWorkflow(TrickWorkflow):
    ...
    def run( self ):
      ...
      first_phase_run_status = self.execute_jobs(first_run_jobs, max_concurrent=self.cpus, header="Executing first phase runs.", job_timeout=300)
      runs_status   = self.execute_jobs(remaining_run_jobs,   max_concurrent=self.cpus, header='Executing remaining runs.', job_timeout=300)
